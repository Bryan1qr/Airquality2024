...
factor_eca <- function(x,y){
  ...
  m1 <- read.csv("input/CR1000_JULIO_2024.csv", skip = 3, na.strings = "NAN") %>% 
    ...
    filter(date %in% seq(
      as.POSIXct(paste("2024-06-21", "00:00:00"), tz = "Etc/GMT"),
      as.POSIXct(paste("2024-06-30", "23:00:00"), tz = "Etc/GMT"),
      by = "1 hour")) %>% 
    mutate_if(is.character, as.numeric) %>% 
    mutate(across(-date, ~ ifelse(. < 0, NA, .)))
  
  g2 <- read.csv("input/gases_odria_agosto.csv", skip = 3, na.strings = "NAN") %>% 
  ...
  p3 <- read.csv(
    "input/pm_junio.csv", skip = 3, na.strings = "NAN") %>% 
