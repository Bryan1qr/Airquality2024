...
El índice de Calidad de Aire (AQI por sus siglas en inglés) muestra los estados de la calidad del aire, en general la zona monitoreada del `r glue(format(min(df$df$date), format = "%d de {fech} del %Y"))` al `r glue(format(max(df$df$date), format = "%d de {fech} del %Y"))` presenta estado de calidad del aire **“Bueno”** en los parámetros $NO_2$, $O_3$, $SO_2$, y $CO$,  durante todos los días de medición. En el caso del $PM_{2.5}$ se observa que del 27 de setiembre al 07 de octubre presenta estado de calidad del aire **“Moderado”**, que puede no ser favorable para personas sensibles a una exposición prolongada de este parámetro en la zona de evaluación. Finalmente, respecto al parámetro$PM_{10}$ este presenta estado de calidad de aire **“Bueno”** en casi todos los días monitoreados, a excepción del día 04 de octubre con estado de calidad del aire **“Moderado”.**
