---
title: "Calidad de aire junio - 2024"
...
+------------+----------+---------------+------------------------------+----------------------------------------------------------+
| Parámetros | Período  | Valor (ug/m3) | Criterios de evaluación      | Método de análisis                                   |
+============+==========+===============+==============================+==========================================================+
| $SO_2$     | 24 horas | 250           | NE más de 7 veces al año | Fluorescencia ultravioleta (método automático)           |
+------------+----------+---------------+------------------------------+----------------------------------------------------------+
...
Durante el `r glue(format(min(df$df$date), format = "%d de {fech} del %Y"))` al `r glue(format(max(df$df$date), format = "%d de {fech} del %Y"))` se realizó el monitoreo de calidad de aire, en `r lugar` del distrito de `r distrito`, utilizando la Estación Móvil de Monitoreo de Calidad de aire del Gobierno Regional de Tacna.

```{r fig.cap=glue("Mapa de Ubicación de la estación {cod}")}
knitr::include_graphics("mapas/output_1.png", dpi = 370)
```
...
<!-- Modificableeeeeeeee -->
En la figura 6 se aprecia que la representación polar de 4 parámetros de calidad de aire de interés $PM_{10}$, $PM_{2.5}$, $NO_2$ y $O_3$. En el caso del parámetro $PM_{10}$ este muestra mayores concentraciones con mayores niveles de velocidad de viento que se encuentran en la región suroeste, en el caso del $PM_{2.5}$ los mayores concentraciones se encuentran donde los niveles de velocidad del viento son mayores, estas concentraciones se situan hacia el noroeste y oeste. Las mayores concentraciones de $NO_2$ se presentan con valores altos de velocidad de viento, hacia la región del suroeste. Los valores de $O_3$ se presentan con valores altos de velocidad de viento con mayor predominancia hacia el noroeste, oeste y suroeste.
